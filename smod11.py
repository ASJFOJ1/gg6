...
class HawkesProcess:
    ...
    def branching_ratio(self) -> float:
        ...
        """
        # FIX: Add validation and better handling
        if self.beta <= 0:
            self.logger.warning("Beta parameter is non-positive, process is unstable")
            return float('inf')
        
        if self.alpha < 0:
            self.logger.warning("Alpha parameter is negative, invalid Hawkes process")
            return 0.0
        
        ratio = self.alpha / self.beta
        
        # FIX: Add stability check and warning
        if ratio >= 1.0:
            self.logger.warning(f"Hawkes process is supercritical (branching ratio={ratio:.3f}). "
                              "Predictions may be unreliable. Process will explode.")
            # Cap the ratio to prevent numerical issues in downstream calculations
            return min(ratio, 10.0)  # Cap at 10 for numerical stability
        elif ratio > 0.9:
            self.logger.warning(f"Hawkes process is near-critical (branching ratio={ratio:.3f}). "
                              "Process is on the edge of stability.")
...
class CNNMarketPredictor:
    ...
    def prepare_data(self, kline_data: Dict[str, pd.DataFrame],
        ...
        self.base_timeframe = list(kline_data.keys())[0] if kline_data else '15m'  # Track base timeframe
        ...
        # For training mode - FIX: Ensure no future leakage and adaptive thresholds
        base_price_series = kline_data[timeframes[0]]['close'].copy()
        
        # Calculate adaptive thresholds based on recent volatility
        volatility_window = min(100, len(base_price_series) // 2)
        if len(base_price_series) > volatility_window:
            recent_returns = base_price_series.pct_change().dropna().iloc[-volatility_window:]
            volatility = recent_returns.std()
            
            # Adaptive thresholds: 1.5 standard deviations
            bull_threshold = volatility * 1.5
            bear_threshold = -volatility * 1.5
            
            # Ensure minimum threshold of 0.5% for noise filtering
            bull_threshold = max(bull_threshold, 0.005)
            bear_threshold = min(bear_threshold, -0.005)
        else:
            # Fallback to fixed thresholds
            bull_threshold = 0.01
            bear_threshold = -0.01
        
        # FIX: Create safe indices for training
        min_len = min(len(combined_features), len(base_price_series))
        
        # Ensure we have enough data for sequence + prediction horizon
        safe_end_idx = min_len - prediction_horizon - 1
        
        if safe_end_idx <= sequence_length:
            return torch.FloatTensor(), torch.LongTensor()
        
        # Create sequences with proper alignment
        for i in range(sequence_length, safe_end_idx):
            # Features from i-sequence_length to i (not including i)
            X.append(combined_features[i-sequence_length:i])
            
            # FIX: Calculate return using prices at exact indices
            current_price = base_price_series.iloc[i]
            future_price = base_price_series.iloc[i + prediction_horizon]
            ...
            if current_price > 0 and future_price > 0:
                ...
                if future_return > bull_threshold:
                    label = 2  # Bull
                elif future_return < bear_threshold:
                    label = 0  # Bear
                else:
                    label = 1  # Neutral
                ...
                # Skip this sample if prices are invalid
                X.pop()

        # Add data quality tracking
        if len(y) > 0:
            label_counts = np.bincount(y, minlength=3)
            self.logger.info(f"Label distribution - Bear: {label_counts[0]}, Neutral: {label_counts[1]}, Bull: {label_counts[2]}")
            self.logger.info(f"Thresholds used - Bull: {bull_threshold:.4f}, Bear: {bear_threshold:.4f}")
            ...
            for i, count in enumerate(label_counts):
                ...
                if pct < 10:
                    self.logger.warning(f"Class {i} has only {pct:.1f}% of samples - consider resampling")
...
class AdvancedMathematicalTools:
    ...
    def multifractal_spectrum_analysis(self, signal: np.ndarray,
        ...
        for scale_idx, scale in enumerate(scales):
            ...
            for q_idx, q in enumerate(q_range):
                ...
                # FIX: Handle zero fluctuations properly
                # Remove zero fluctuations to avoid log(0)
                nonzero_fluct = fluctuations_arr[fluctuations_arr > 0]
                
                if len(nonzero_fluct) == 0:
                    # If all fluctuations are zero, set Fq to a small value
                    Fq[q_idx, scale_idx] = 1e-10
                elif q == 0:
                    # FIX: Use only non-zero fluctuations for q=0 case
                    Fq[q_idx, scale_idx] = np.exp(0.5 * np.mean(np.log(nonzero_fluct ** 2)))
                else:
                    # For other q values, include zero fluctuations but handle them properly
                    if np.any(fluctuations_arr == 0) and q < 0:
                        # For negative q, zero fluctuations would cause infinity
                        # Use only non-zero fluctuations
                        Fq[q_idx, scale_idx] = np.mean(nonzero_fluct ** q) ** (1 / q)
                    else:
                        # For positive q, zero fluctuations are fine
                        Fq[q_idx, scale_idx] = np.mean(fluctuations_arr ** q) ** (1 / q) if np.mean(fluctuations_arr ** q) > 0 else 1e-10
        ...
        for q_idx in range(len(q_range)):
            ...
            if np.sum(valid_idx) > 1:
                ...
                # FIX: Use robust regression to handle outliers
                try:
                    # Remove outliers using median absolute deviation
                    residuals = log_Fq - np.median(log_Fq)
                    mad = np.median(np.abs(residuals))
                    outlier_threshold = 3 * mad / 0.6745  # 3 sigma equivalent
                    
                    clean_mask = np.abs(residuals) < outlier_threshold
                    if np.sum(clean_mask) > 2:
                        hurst_q[q_idx] = np.polyfit(log_scales[clean_mask], log_Fq[clean_mask], 1)[0]
                    else:
                        hurst_q[q_idx] = np.polyfit(log_scales, log_Fq, 1)[0]
                except:
                    hurst_q[q_idx] = 0.5  # Default to random walk

        # Multifractal spectrum
        tau_q = q_range * hurst_q - 1

        # Legendre transform
        # FIX: Use numerical differentiation with smoothing
        from scipy.ndimage import gaussian_filter1d
        
        # Smooth tau_q before differentiation to reduce noise
        tau_q_smooth = gaussian_filter1d(tau_q, sigma=1.0, mode='nearest')
        
        # Calculate alpha using gradient
        alpha = np.gradient(tau_q_smooth, q_range)
        
        # Calculate f(alpha)
        f_alpha = q_range * alpha - tau_q_smooth

        # Width of spectrum (multifractality measure)
        # FIX: Ensure we have valid alpha values
        ...
        # FIX: Add validation for results
        # Check if the spectrum is physically meaningful
        if alpha_range > 10 or alpha_range < 0:
            # Unrealistic multifractality, likely numerical error
            alpha_range = np.clip(alpha_range, 0, 2)
        ...
        return {
            ...
            'scales': scales,
            'Fq': Fq,
            'spectrum_validity': alpha_range > 0 and alpha_range < 2  # Sanity check
        }
    ...
    def kyle_lambda_estimation(self, price_changes: np.ndarray,
        ...
        # FIX: Add stricter data requirements and validation
        min_required_points = 30  # Increased from 10 for statistical reliability
        
        # Enhanced NaN guards with proper filtering
        ...
        if mask.any():
            # Remove zero price changes (no information)
            mask &= (price_changes != 0)
            # Remove extreme outliers (likely data errors)
            if mask.sum() > 10:
                price_z_scores = np.abs((price_changes[mask] - np.median(price_changes[mask])) / 
                                      (np.median(np.abs(price_changes[mask] - np.median(price_changes[mask]))) * 1.4826))
                volume_z_scores = np.abs((signed_volume[mask] - np.median(signed_volume[mask])) / 
                                       (np.median(np.abs(signed_volume[mask] - np.median(signed_volume[mask]))) * 1.4826))
                # Keep points within 5 MAD (Median Absolute Deviation)
                outlier_mask = (price_z_scores < 5) & (volume_z_scores < 5)
                # Update the main mask
                temp_mask = mask.copy()
                temp_mask[mask] &= outlier_mask
                mask = temp_mask
        
        if mask.sum() < min_required_points:
            self.logger.warning(f"Insufficient data for Kyle Lambda: {mask.sum()} < {min_required_points}")
            return {
                'lambda': 0.0, 
                'r_squared': 0.0, 
                'market_depth': np.inf,
                'n_valid_points': mask.sum(),
        ...
        if method == 'regression':
            # Enhanced OLS regression with outlier detection
            X = signed_volume.reshape(-1, 1)
            y = price_changes

            # Calculate using robust regression statistics
            n = len(X)
            ...
            residuals = y - y_pred
            se_lambda = np.sqrt(np.sum(residuals**2) / (n - 1) / (XtX))
            
            # T-statistic for significance test
            t_stat = lambda_estimate / (se_lambda + 1e-10)
            
            # Durbin-Watson statistic for autocorrelation check
            dw_stat = np.sum(np.diff(residuals)**2) / (ss_res + 1e-10)

        elif method == 'robust':
            # Enhanced robust estimation using Theil-Sen estimator
            # More resistant to outliers than simple median
            n_samples = min(1000, len(price_changes))  # Limit for computational efficiency
            ...
            if len(price_changes) > 50:
                ...
                for _ in range(n_samples):
                    ...
                    idx = np.random.choice(len(price_changes), 2, replace=False)
                    dv = signed_volume[idx[1]] - signed_volume[idx[0]]
                    dp = price_changes[idx[1]] - price_changes[idx[0]]
                    
                    if abs(dv) > 1e-10:  # Avoid division by zero
                        lambdas.append(dp / dv)
                
                if lambdas:
                    # Use median of slopes (Theil-Sen estimator)
                    lambda_estimate = np.median(lambdas)
                    
                    # Calculate MAD for robustness measure
                    mad = np.median(np.abs(lambdas - lambda_estimate))
                    
                    # Robust confidence interval
                    ci_lower = np.percentile(lambdas, 2.5)
                    ci_upper = np.percentile(lambdas, 97.5)
                else:
                    lambda_estimate = 0.0
                    mad = 0.0
                    ci_lower = ci_upper = 0.0
            else:
                # Fallback to simple robust estimation
                ratios = price_changes / (signed_volume + 1e-10)
                finite_ratios = ratios[np.isfinite(ratios)]
                lambda_estimate = np.median(finite_ratios) if len(finite_ratios) > 0 else 0.0
                mad = np.median(np.abs(finite_ratios - lambda_estimate)) if len(finite_ratios) > 1 else 0.0
                ci_lower = ci_upper = lambda_estimate

            # Pseudo R-squared for robust method
            y_pred = lambda_estimate * signed_volume
            # Use median absolute deviation for robust R-squared
            mad_residual = np.median(np.abs(price_changes - y_pred))
            mad_total = np.median(np.abs(price_changes - np.median(price_changes)))
            r_squared = 1 - (mad_residual / (mad_total + 1e-10)) if mad_total > 0 else 0.0
            r_squared = max(0, min(1, r_squared))  # Ensure between 0 and 1
            
            # Additional robust statistics
            se_lambda = mad * 1.4826  # MAD to standard deviation
            t_stat = lambda_estimate / (se_lambda + 1e-10)
            dw_stat = np.nan  # Not applicable for robust method
        ...
        # Enhanced Amihud illiquidity (with volume weighting)
        abs_volume = np.abs(signed_volume)
        if abs_volume.sum() > 0:
            # Volume-weighted Amihud
            weights = abs_volume / abs_volume.sum()
            amihud = np.sum(weights * np.abs(price_changes) / (abs_volume + 1e-10))
        else:
            # Simple Amihud
            amihud = np.mean(np.abs(price_changes) / (abs_volume + 1e-10))

        # Estimate permanent vs temporary impact (simplified)
        if len(price_changes) > 10:
            # Check if price impact reverses
            cum_price_change = np.cumsum(price_changes)
            final_impact = cum_price_change[-1]
            max_impact = np.max(np.abs(cum_price_change))
            permanence_ratio = abs(final_impact) / (max_impact + 1e-10) if max_impact > 0 else 0.0
        else:
            permanence_ratio = 1.0  # Assume permanent with limited data
        ...
        result = {
            ...
            'n_valid_points': len(price_changes),
            'outliers_removed': mask.sum() - len(price_changes) if 'mask' in locals() else 0,
            'permanence_ratio': permanence_ratio,
            'method_used': method
        }
        ...
        if method == 'regression':
            result.update({
                ...
                'significant': abs(t_stat) > 2.0  # 5% significance level
            })
        elif method == 'robust':
            result.update({
                'median_absolute_deviation': mad if 'mad' in locals() else 0.0,
                'confidence_interval': (ci_lower, ci_upper) if 'ci_lower' in locals() else (0.0, 0.0),
                'robust_std_error': se_lambda if 'se_lambda' in locals() else 0.0
            })
    ...
    def _calculate_transfer_entropy(self, x_past: np.ndarray,
        ...
        """Helper function to calculate transfer entropy with improved bias correction"""
        ...
        if min_len < 20:  # FIX: Increase minimum sample requirement
            return 0.0
        ...
        # FIX: Improved bias correction for small samples
        # Use Grassberger correction which is more accurate for small samples
        grassberger_correction = 1.0 / (2 * n_samples * np.log(2))
        
        # Calculate entropies with Grassberger correction
        epsilon = 1e-12
        
        def grassberger_entropy(probs, n_bins):
            """Calculate entropy with Grassberger correction"""
            h = 0.0
            for p in probs:
                if p > epsilon:
                    # Grassberger correction term
                    psi_term = 0.5772156649  # Euler-Mascheroni constant
                    if n_samples * p > 1:
                        # Use digamma function approximation
                        psi_approx = np.log(n_samples * p) - 1/(2 * n_samples * p)
                    else:
                        psi_approx = psi_term
                    h -= p * (np.log2(p) - grassberger_correction * psi_approx)
            return h
        
        h_xyz = grassberger_entropy(p_xyz, n_bins_xyz)
        h_yz = grassberger_entropy(p_yz, n_bins_yz)
        h_yx = grassberger_entropy(p_yx, n_bins_yx)
        h_y = grassberger_entropy(p_y, n_bins_y)
        
        # Transfer Entropy calculation
        te = h_yz + h_yx - h_y - h_xyz
        
        # FIX: Improved significance testing using permutation test
        if n_samples < 100:
            # For small samples, use permutation test
            n_permutations = min(100, int(np.math.factorial(min(5, n_samples))))
            permuted_te_values = []
            
            for _ in range(n_permutations):
                # Shuffle x_past to break temporal relationship
                x_shuffled = np.random.permutation(x_past)
                
                # Recalculate TE with shuffled data
                xyz_shuf = np.vstack([y_future, y_past, x_shuffled]).T
                yx_shuf = np.vstack([y_past, x_shuffled]).T
                
                p_xyz_shuf = self._get_probs(xyz_shuf)
                p_yx_shuf = self._get_probs(yx_shuf)
                
                if len(p_xyz_shuf) > 0 and len(p_yx_shuf) > 0:
                    h_xyz_shuf = grassberger_entropy(p_xyz_shuf, len(p_xyz_shuf))
                    h_yx_shuf = grassberger_entropy(p_yx_shuf, len(p_yx_shuf))
                    
                    te_shuf = h_yz + h_yx_shuf - h_y - h_xyz_shuf
                    permuted_te_values.append(te_shuf)
            
            if permuted_te_values:
                # Calculate p-value
                te_mean = np.mean(permuted_te_values)
                te_std = np.std(permuted_te_values)
                
                # If TE is not significantly different from shuffled, return 0
                if te_std > 0:
                    z_score = (te - te_mean) / te_std
                    if abs(z_score) < 1.96:  # 95% confidence
                        return 0.0
        
        # FIX: Apply Panzeri-Treves correction more carefully
        # Only apply if we have sufficient samples
        if n_samples > 50:
            pt_correction = (n_bins_xyz - n_bins_yz - n_bins_yx + n_bins_y) / (2 * n_samples * np.log(2))
            te -= pt_correction
        
        # FIX: Handle negative TE values more intelligently
        if te < 0:
            # For very small negative values, this is likely noise
            if abs(te) < 0.001:
                return 0.0
            
            # For larger negative values, check if it's statistically significant
            ...
            if n_samples > 30:
                ...
                for _ in range(n_bootstrap):
                    ...
                    x_boot = x_past[indices]
                    y_boot = y_past[indices]
                    yf_boot = y_future[indices]
                    
                    # Recalculate for bootstrap sample
                    xyz_boot = np.vstack([yf_boot, y_boot, x_boot]).T
                    yz_boot = np.vstack([yf_boot, y_boot]).T
                    yx_boot = np.vstack([y_boot, x_boot]).T
                    y_boot_single = y_boot.reshape(-1, 1)
                    
                    p_xyz_boot = self._get_probs(xyz_boot)
                    p_yz_boot = self._get_probs(yz_boot)
                    p_yx_boot = self._get_probs(yx_boot)
                    p_y_boot = self._get_probs(y_boot_single)
                    
                    if all(len(p) > 0 for p in [p_xyz_boot, p_yz_boot, p_yx_boot, p_y_boot]):
                        h_xyz_boot = grassberger_entropy(p_xyz_boot, len(p_xyz_boot))
                        h_yz_boot = grassberger_entropy(p_yz_boot, len(p_yz_boot))
                        h_yx_boot = grassberger_entropy(p_yx_boot, len(p_yx_boot))
                        h_y_boot = grassberger_entropy(p_y_boot, len(p_y_boot))
                        
                        te_boot = h_yz_boot + h_yx_boot - h_y_boot - h_xyz_boot
                        bootstrap_te.append(te_boot)
                
                if bootstrap_te:
                    # Calculate confidence interval
                    te_lower = np.percentile(bootstrap_te, 2.5)
                    te_upper = np.percentile(bootstrap_te, 97.5)
                    
                    # If 0 is within confidence interval, return 0
                    if te_lower <= 0 <= te_upper:
                        return 0.0
...
class MicrostructureKlineBridge:
    ...
    def calculate_prediction_confidence(self, predictions: Dict[str, float],
        ...
        # Use statistical approach for confidence adjustment
        # Based on coefficient of variation thresholds from literature
        cv_thresholds = {
            'very_low': 0.05,  # 95th percentile agreement
            'low': 0.15,       # 85th percentile
            'medium': 0.30,    # 70th percentile
            'high': 0.50       # 50th percentile
        }
        
        # Map CV to confidence multiplier using logistic function
        def cv_to_multiplier(cv):
            # Sigmoid function centered at cv=0.25
            return 1 / (1 + np.exp(10 * (cv - 0.25)))
        
        # Calculate confidence adjustment
        confidence_multiplier = 0.5 + 0.5 * cv_to_multiplier(cv)
...
class IntegratedMathematicalAnalysisSystem:
    ...
    def collect_all_market_data_by_endpoint(self):
        ...
        for func, name in tasks_to_run:
            ...
            with ThreadPoolExecutor(max_workers=self.n_processes) as executor:
                futures = {
                    executor.submit(func, timeframe=tf): (tf, name)
                    for tf in self.timeframes
                }

                # Process results as they complete
                for future in as_completed(futures):
                    timeframe, task_name = futures[future]
                    try:
                        future.result(timeout=180)
                        # Track success for rate limiter
                        self.rate_limiter.success_count += 1
                    except Exception as e:
                        self.logger.exception(f"Failed {task_name} for {timeframe}")
                        self.log_error(task_name, {'timeframe': timeframe}, str(e), f"Data collection for {task_name}")
                        failed_tasks.append((task_name, timeframe, str(e)))
                        # Track error for rate limiter
                        self.rate_limiter.error_count += 1

        # Add summary of failures
        if failed_tasks:
            print(f"\n⚠ Failed tasks: {len(failed_tasks)}")
            for task, tf, error in failed_tasks[:5]:  # Show first 5
                print(f"  - {task} ({tf}): {error[:50]}...")
    ...
    def perform_comprehensive_analysis_enhanced(self):
            ...
            try:
                ...
                if of_data:
                    ...
                    if 'timestamp' in of_df.columns:
                        ...
                        of_df = pd.DataFrame(index=price_df.index, columns=['orderflow'])
                else:
                    of_df = pd.DataFrame(index=price_df.index, columns=['orderflow'])
                ...
                if cvd_data:
                    ...
                    if 'timestamp' in cvd_df.columns:
                        ...
                        cvd_df = pd.DataFrame(index=price_df.index, columns=['cvd'])
                else:
                    cvd_df = pd.DataFrame(index=price_df.index, columns=['cvd'])

                # FIX: Use proper alignment without forward fill before VECM
                # First align indices
                common_index = price_df.index.intersection(of_df.index).intersection(cvd_df.index)
                
                if len(common_index) > 20:
                    # Use only common timestamps
                    vecm_data = pd.DataFrame({
                        'close': price_df.loc[common_index, 'close'],
                        'orderflow': of_df.loc[common_index, 'orderflow'],
                        'cvd': cvd_df.loc[common_index, 'cvd']
                    })
                    ...
                    if len(vecm_data) > 20 and vecm_data.shape[1] >= 2:
                        ...
                        self.analysis_results[timeframe]['vecm'] = {'error': 'Insufficient aligned data for VECM after cleaning.'}
                else:
                    # If not enough common timestamps, use interpolation instead of forward fill
                    vecm_data = price_df.copy()
                    
                    # Interpolate missing values instead of forward fill
                    if not of_df.empty:
                        of_series = of_df['orderflow'].reindex(price_df.index).interpolate(method='time', limit=3)
                        vecm_data['orderflow'] = of_series
                    else:
                        vecm_data['orderflow'] = 0
                    
                    if not cvd_df.empty:
                        cvd_series = cvd_df['cvd'].reindex(price_df.index).interpolate(method='time', limit=3)
                        vecm_data['cvd'] = cvd_series
                    else:
                        vecm_data['cvd'] = 0
                    ...
                    if len(vecm_data) > 20:
                        ...
                        self.analysis_results[timeframe]['vecm'] = {'error': 'Insufficient data for VECM after interpolation.'}
        ...
        try:
            ...
            print(f"  ✗ Memory error in tensor fusion: {str(e)}")
            self.logger.error(f"Memory error in tensor fusion: {str(e)}")
            self.analysis_results['tensor_fusion'] = {'error': 'Memory allocation failed', 'tensor_shape': (0, 0, 0)}
        except Exception as e:
            print(f"  ✗ Error in tensor fusion: {str(e)}")
            self.logger.exception("Tensor fusion error")
            self.analysis_results['tensor_fusion'] = {'error': str(e), 'tensor_shape': (0, 0, 0)}
        ...
        try:
            ...
            print(f"  ✗ Error in information flow analysis: {str(e)}")
        ...
        try:
            ...
            print(f"  ✗ Error in CMI analysis: {str(e)}")
        ...
        try:
            ...
            print(f"  ✗ Error in microstructure bridge: {str(e)}")
        ...
        if not self.load_existing_models:
            ...
            try:
                ...
                self.logger.exception("CNN training error")
            ...
            try:
                ...
                self.logger.exception("CNN prediction error")
                self.analysis_results['cnn_prediction'] = {'error': str(e)}
        ...
        try:
            ...
            print(f"  ✗ Error in order flow analysis: {str(e)}")
            self.logger.exception("Order flow analysis error")
        ...
        try:
            ...
            self.logger.exception("Positioning analysis error")

        print("23. Analyzing Liquidation Cascades...")
        try:
            self.analyze_liquidation_cascades_enhanced()
        except Exception as e:
            print(f"  ✗ Error in liquidation analysis: {str(e)}")
            self.logger.exception("Liquidation analysis error")
        ...
        try:
            ...
            self.logger.exception("Summary generation error")
            # Write basic error summary
            with open(self.summary_file, 'w', encoding='utf-8') as f:
                f.write(f"ERROR GENERATING SUMMARY\n")
    ...
    def perform_microstructure_bridge_analysis(self):
        """Bridge microstructure data to kline analysis"""
        try:
            ...
            if '15m' in self.kline_data:
                ...
                if '15m' in self.collected_data and 'order_flow' in self.collected_data['15m']:
                    ...
                    if not orderflow_data.empty:
                        ...
                        if 'total_bid' in orderflow_data.columns and 'total_ask' in orderflow_data.columns:
                            recent_data = orderflow_data.tail(20)  # Last 20 records
                            total_bid = recent_data['total_bid'].sum()
                            total_ask = recent_data['total_ask'].sum()
                            
                            if total_bid + total_ask > 0:
                                bid_ratio = total_bid / (total_bid + total_ask)
                                orderflow_analysis = {
                                    'bid_ratio': bid_ratio,
                                    'net_flow': total_bid - total_ask,
                                    'total_volume': total_bid + total_ask,
                                    'flow_imbalance': (total_bid - total_ask) / (total_bid + total_ask)
                                }
                ...
                if '15m' in self.collected_data and 'orderbook' in self.collected_data['15m']:
                    ...
                    if not book_df.empty:
                        ...
                        if 'buy_usd' in latest_book and 'sell_usd' in latest_book:
                            buy_usd = float(latest_book['buy_usd'])
                            sell_usd = float(latest_book['sell_usd'])
                            
                            if buy_usd + sell_usd > 0:
                                book_imbalance = (buy_usd - sell_usd) / (buy_usd + sell_usd)
                                book_analysis = {
                                    'microprice': microprice,
                                    'book_imbalance': book_imbalance,
                                    'bid_size': buy_usd,
                                    'ask_size': sell_usd,
                                    'total_depth': buy_usd + sell_usd
                                }

                # Calculate price position within range
                price_range = current_bar['high'] - current_bar['low']
                if price_range > 0:
                    range_position = (current_bar['current'] - current_bar['low']) / price_range
                else:
                    range_position = 0.5
                ...
                # Order flow pressure
                if orderflow_analysis:
                    if orderflow_analysis['flow_imbalance'] > 0.2:
                        pressure_factors.append("Strong buying flow")
                        pressure_score += 2
                    elif orderflow_analysis['flow_imbalance'] > 0.1:
                        pressure_factors.append("Buying flow")
                        pressure_score += 1
                    elif orderflow_analysis['flow_imbalance'] < -0.2:
                        pressure_factors.append("Strong selling flow")
                        pressure_score -= 2
                    elif orderflow_analysis['flow_imbalance'] < -0.1:
                        pressure_factors.append("Selling flow")
                        pressure_score -= 1
                    else:
                        pressure_factors.append("Balanced flow")
                
                # Book pressure
                if book_analysis:
                    if book_analysis['book_imbalance'] > 0.3:
                        pressure_factors.append("Heavy bid pressure")
                        pressure_score += 2
                    elif book_analysis['book_imbalance'] > 0.1:
                        pressure_factors.append("Bid pressure")
                        pressure_score += 1
                    elif book_analysis['book_imbalance'] < -0.3:
                        pressure_factors.append("Heavy ask pressure")
                        pressure_score -= 2
                    elif book_analysis['book_imbalance'] < -0.1:
                        pressure_factors.append("Ask pressure")
                        pressure_score -= 1
                    else:
                        pressure_factors.append("Balanced book")
                
                # Range position pressure
                if range_position > 0.8:
                    pressure_factors.append("Near highs")
                    pressure_score += 1
                elif range_position < 0.2:
                    pressure_factors.append("Near lows")
                    pressure_score -= 1
                else:
                    pressure_factors.append("Mid-range")
                
                # Overall market pressure
                if pressure_score >= 3:
                    market_pressure = "STRONG_BULLISH"
                elif pressure_score >= 1:
                    market_pressure = "BULLISH"
                elif pressure_score <= -3:
                    market_pressure = "STRONG_BEARISH"
                elif pressure_score <= -1:
                ...
                data_quality = {
                    'orderflow_records': len(orderflow_data),
                    'orderbook_records': len(book_data) if isinstance(book_data, pd.DataFrame) else 0,
                    'has_realtime_data': len(orderflow_data) > 0 or (isinstance(book_data, pd.DataFrame) and len(book_data) > 0)
                }
                ...
                self.analysis_results['microstructure_bridge'] = {
                    ...
                    'orderflow_metrics': orderflow_analysis,
                    'orderbook_metrics': book_analysis,
                    'data_quality': data_quality
                }

                # Print analysis results
                print(f"  ✓ Current price: ${current_bar['current']:,.2f}")
                print(f"  ✓ Price position: {range_position*100:.1f}% of daily range")
                print(f"  ✓ Market pressure: {market_pressure}")
                print(f"  ✓ Key factors: {', '.join(pressure_factors[:3])}")
                
                if orderflow_analysis:
                    print(f"  ✓ Order flow: {orderflow_analysis['bid_ratio']*100:.1f}% buying")
                
                if book_analysis:
                    print(f"  ✓ Book imbalance: {book_analysis['book_imbalance']:.3f}")
                
                print(f"  ✓ Data quality: {data_quality['orderflow_records']} orderflow, "
                      f"{data_quality['orderbook_records']} orderbook records")

            else:
                print("  ✗ No 15m kline data available for microstructure analysis")
                self.analysis_results['microstructure_bridge'] = {
                    'error': 'No 15m kline data available'
                }
        ...
        for timeframe in self.timeframes[:3]:  # Focus on shorter timeframes
            ...
            for key, records in exchange_data.items():
                ...
                if total_ask_volume + total_bid_volume > 0:
                    ...
                    # FIX: Adaptive timestamp alignment for Kyle Lambda
                    if timeframe in self.kline_data:
                        # Get timestamps from records with validation
                        record_timestamps = pd.to_datetime([r['timestamp'] for r in records], unit='ms')
                        
                        # Create a more robust alignment
                        price_series = self.kline_data[timeframe]['close']
                        
                        # Ensure no future data leakage
                        current_time_pd = pd.Timestamp(self.current_timestamp, unit='ms')
                        price_series = price_series[price_series.index <= current_time_pd]
                        
                        # FIX: Adaptive tolerance based on timeframe
                        timeframe_seconds = {
                            '15m': 900, '30m': 1800, '1h': 3600, '4h': 14400,
                            '6h': 21600, '12h': 43200, '1d': 86400
                        }.get(timeframe, 3600)
                        
                        # Use 5% of timeframe duration or 60 seconds, whichever is smaller
                        tolerance_seconds = min(timeframe_seconds * 0.05, 60)
                        ...
                        for i, ts in enumerate(record_timestamps):
                            ...
                            time_diff = abs(price_series.index - ts)
                            if time_diff.min() < pd.Timedelta(seconds=tolerance_seconds):
                                # FIX: Use np.argmin() to find the integer position of the minimum value,
                                # then use that position to get the index label from the original series.
                                closest_pos = np.argmin(time_diff)
                                closest_idx = price_series.index[closest_pos]

                                aligned_prices.append(price_series.loc[closest_idx])
                                aligned_records.append(records[i])
                        
                        # FIX: Require more data points for reliable Kyle Lambda
                        min_required_points = max(30, len(records) * 0.3)  # At least 30 points or 30% of data
                        
                        if len(aligned_prices) >= min_required_points:
                            # Use aligned data for Kyle Lambda
                            aligned_prices = pd.Series(aligned_prices)
                            price_changes = np.diff(aligned_prices.values)
                            
                            # Calculate signed volume from aligned records
                            signed_volume = np.array([
                                r['total_bid'] - r['total_ask'] for r in aligned_records[1:]
                            ])
                            ...
                            if mask.sum() >= min_required_points:
                                ...
                                # Calculate Kyle Lambda with robust method
                                kyle_result = self.advanced_tools.kyle_lambda_estimation(
                                    price_changes,
                                    signed_volume,
                                    method='robust'  # Use robust estimation
                                )
                                ...
                                alignment_rate = len(aligned_prices)/len(records)*100
                                self.write_to_file(f"  Alignment Rate: {alignment_rate:.1f}%")
                                self.write_to_file(f"  Valid Data Points: {mask.sum()}/{len(aligned_prices)-1}")
                                self.write_to_file(f"  Tolerance Window: ±{tolerance_seconds}s")
                                ...
                                if kyle_result['r_squared'] < 0.1:
                                    self.write_to_file(f"  ⚠️ WARNING: Low R² suggests weak price-volume relationship")
                            else:
                                self.write_to_file(f"  Kyle Lambda: Insufficient clean data ({mask.sum()} < {min_required_points})")
                        else:
                            self.write_to_file(f"  Kyle Lambda: Insufficient aligned data ({len(aligned_prices)} < {min_required_points})")
                            self.write_to_file(f"  Alignment Rate: {len(aligned_prices)/len(records)*100:.1f}%")
                    ...
                    if len(imbalances) >= 20:
                        ...
                        if len(volumes) > 0:
                            ...
                            vol_shares = [v/total_vol for v in volumes]
                            herfindahl = sum(s**2 for s in vol_shares)
                            self.write_to_file(f"  Flow Concentration (HHI): {herfindahl:.3f}")
                            if herfindahl > 0.15:
                                self.write_to_file(f"    → High concentration, large players dominating")
    ...
    def generate_comprehensive_summary_enhanced(self):
        ...
        with open(self.summary_file, 'w', encoding='utf-8') as f:
            ...
            if 'cnn_prediction' in self.analysis_results and 'error' not in self.safe_get_nested(self.analysis_results, ['cnn_prediction'], {}):
                ...
                # FIX: Get actual timeframe from CNN predictor
                base_timeframe = self.cnn_predictor.base_timeframe if hasattr(self.cnn_predictor, 'base_timeframe') else '15m'
                timeframe_minutes = {
                    '15m': 15, '30m': 30, '1h': 60, '4h': 240,
                    '6h': 360, '12h': 720, '1d': 1440
                }.get(base_timeframe, 15)
                
                prediction_horizon = getattr(self.cnn_predictor, 'prediction_horizon', 5)
                horizon_hours = prediction_horizon * timeframe_minutes / 60
            ...
            if ('15m' in self.kline_data and not self.kline_data['15m'].empty and 
                'close' in self.kline_data['15m'].columns):
    ...
    def calculate_weighted_bias(self, timeframe_signals, tf_weights):
        ...
        # Return None if no data instead of defaulting to 50%
        if weighted_total == 0:
            return None
        
        return (weighted_bullish / weighted_total) * 100
