...
class SimplifiedTensorAnalysis:
    ...
    def create_simplified_tensor(self, data: Dict[str, pd.DataFrame]) -> np.ndarray:
        ...
        valid_tfs = [tf for tf in timeframes if tf in data and not data[tf].empty]
        if not valid_tfs:
...
class HawkesProcess:
    ...
    def fit(self, event_times: np.ndarray, max_iter: int = 100, tol: float = 1e-6):
        """
        Fit Hawkes process parameters using Maximum Likelihood Estimation
        """
        ...
        for iteration in range(max_iter):
            ...
            # E-step: Calculate intensities
            intensities = self._calculate_intensities(event_times)
            ...
            for i in range(1, n_events):
                R_i = 0
                for j in range(i):
                    R_i += np.exp(-self.beta * (event_times[i] - event_times[j]))
                sum_R += R_i

                for j in range(i):
                    delta_t = event_times[i] - event_times[j]
                    sum_R_delta += delta_t * np.exp(-self.beta * delta_t)

            if sum_R > 0:
                self.alpha = (n_events - self.mu * T) / sum_R

            if sum_R_delta > 0 and sum_R > 0:
                self.beta = sum_R / sum_R_delta

            # Check convergence
            param_change = abs(self.mu - old_mu) + abs(self.alpha - old_alpha) + abs(self.beta - old_beta)
            if param_change < tol:
                break
    ...
    def simulate(self, T: float, n_simulations: int = 1) -> List[np.ndarray]:
        ...
        for _ in range(n_simulations):
            ...
            while t < T and iterations < max_iterations:
                ...
                t += np.random.exponential(1 / max(intensity_upper, 1e-6))
                iterations += 1
                ...
                if np.random.rand() < true_intensity / intensity_upper:
                    events.append(t)
    ...
    def branching_ratio(self) -> float:
        """
        Calculate branching ratio (α/β) - if > 1, process is explosive
        """
...
class UnscentedKalmanFilter:
    ...
    def generate_sigma_points(self, x: np.ndarray, P: np.ndarray) -> np.ndarray:
        ...
        # Add larger regularization to ensure positive definiteness
        eps = 1e-3
        P_reg = P + eps * np.eye(n)

        # Try multiple approaches to handle numerical issues
        try:
            # First attempt: Direct Cholesky
            S = np.linalg.cholesky((n + self.lambda_) * P_reg)
        except np.linalg.LinAlgError:
            try:
                # Second attempt: SVD-based approach
                U, s, Vt = np.linalg.svd(P_reg)
                s = np.maximum(s, eps)  # Ensure all eigenvalues are positive
                S = U @ np.diag(np.sqrt(s * (n + self.lambda_)))
            except:
                # Final fallback: Use identity matrix scaled by trace
                trace_P = np.trace(P)
...
class AdvancedMathematicalTools:
    ...
    def cointegration_and_vecm(self, data: pd.DataFrame, lag: int = 1) -> Dict[str, Any]:
        ...
        # Cointegration test
        try:
            # Test between first two columns as a primary check
    ...
    def _calculate_transfer_entropy(self, x_past: np.ndarray,
        ...
        """
        Helper function to calculate transfer entropy.
    ...
    def sample_entropy(self, signal: np.ndarray,
        ...
        if normalize:
            ...
            if signal_std > 1e-10:
                ...
                return {'sample_entropy': 0.0, 'threshold': 0.0, 'A': 0, 'B': 0}
...
class MicrostructureKlineBridge:
    ...
    def predict_current_bar_close(self,
        ...
        valid_predictions = {k: v for k, v in predictions.items() if v is not None and v > 0 and np.isfinite(v)}
    ...
    def calculate_prediction_confidence(self, predictions: Dict[str, float],
                                      current_bar_data: Dict[str, Any]) -> float:
        """Calculate confidence in prediction with proper numerical handling"""
        ...
        confidence = (1 - cv/2) * time_confidence * 100
...
class IntegratedMathematicalAnalysisSystem:
    ...
    def perform_comprehensive_analysis_enhanced(self):
        ...
        for timeframe in self.timeframes:
            ...
            try:
                ...
                vecm_data = vecm_data.fillna(method='ffill').dropna()
    ...
    def eemd(self, signal: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            eemd_analyzer = EEMD(parallel=False)
            ...
            self.logger.exception("EEMD analysis error")
            return {
                'n_imfs': 0,
                'dominant_periods': [],
                'energy_distribution': [],
    ...
    def hidden_markov_model(self, returns: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            state_vars = np.sqrt(np.array([cov[0][0] for cov in model.covars_]))
            ...
            log_prob, state_sequence = model.decode(X)

            return {
                'current_state': current_label,
                'state_probabilities': model.predict_proba(X)[-1].tolist(),
    ...
    def analyze_liquidation_hawkes(self, timeframe: str) -> Dict[str, Any]:
        ...
        if 'total_value' in liq_data.columns and liq_data['total_value'].nunique() > 1:
            ...
            significant_events = event_times[liq_data['total_value'] > threshold]
        else:
        ...
        try:
            ...
            for timeframe in self.timeframes:
                ...
                if aligned_dfs:
                    ...
                    # Add required columns if missing for compatibility with create_simplified_tensor
                    for col in ['open', 'high', 'low', 'volume']:
                        if col not in combined_df.columns and col in self.kline_data[timeframe].columns:
                            combined_df = combined_df.join(self.kline_data[timeframe][[col]])

                    if not combined_df.empty:
                        tensor_data[timeframe] = combined_df.fillna(0)
    ...
    def _prepare_aligned_data(self, timeframe: str) -> Dict[str, pd.DataFrame]:
        """Helper to prepare timestamp-aligned data for a given timeframe - FIXED."""
        aligned_data = {}
        if timeframe not in self.kline_data:
            return aligned_data
                ...
                if not df_raw.empty and 'timestamp' in df_raw.columns:
                    ...
                    for old_name, new_name in col_map.items():
                        if old_name in df_proc.columns:
                            # Reindex to match base_df index and fill
                            series = df_proc[old_name].reindex(base_df.index, method='ffill').fillna(0)
        ...
        for timeframe in self.timeframes[:3]:  # Focus on shorter timeframes
            ...
            for key, records in exchange_data.items():
                ...
                if total_ask_volume + total_bid_volume > 0:
                    ...
                    # FIX: Match by timestamp for Kyle Lambda
                    if timeframe in self.kline_data:
                        ...
                        # Align prices with record timestamps
                        price_series = self.kline_data[timeframe]['close']
                        aligned_prices = price_series.reindex(record_timestamps, method='nearest').dropna()
    ...
    def train_microstructure_model(self, lookback_periods: int = 100):
        ...
        # Collect training data from multiple periods
        X_train_list = []
        y_train_list = []
        
        # Use 15m timeframe for training
        if '15m' not in self.kline_data:
            print("  ✗ No 15m data available for training")
            self.logger.warning("No 15m data available for XGBoost training")
            return
        
        kline_df = self.kline_data['15m']
        
        # Prepare order flow and book data if available
        orderflow_df = pd.DataFrame()
        if '15m' in self.collected_data and 'order_flow' in self
        orderflow_df = pd.DataFrame()
        if '15m' in self.collected_data and 'order_flow' in self.collected_data['15m']:
            df_raw = pd.DataFrame(self.collected_data['15m']['order_flow'])
            if not df_raw.empty and 'timestamp' in df_raw.columns:
                df_raw['timestamp'] = pd.to_datetime(df_raw['timestamp'], unit='ms')
                orderflow_df = df_raw.set_index('timestamp')

        book_df = pd.DataFrame()
        if '15m' in self.collected_data and 'orderbook' in self.collected_data['15m']:
            df_raw = pd.DataFrame(self.collected_data['15m']['orderbook'])
            if not df_raw.empty and 'timestamp' in df_raw.columns:
                df_raw['timestamp'] = pd.to_datetime(df_raw['timestamp'], unit='ms')
                book_df = df_raw.set_index('timestamp')

        if orderflow_df.empty or book_df.empty:
            print("  ✗ No microstructure data available for training")
            self.logger.warning("No orderflow or book data for XGBoost training")
            return

        print(f"  Training on {lookback_periods} historical 15m bars...")

        # Iterate through historical bars to create a training set
        for i in range(len(kline_df) - lookback_periods, len(kline_df) - 1):
            bar_start = kline_df.index[i]
            bar_end = kline_df.index[i+1]
            
            # Simulate being partway through the bar
            time_into_bar = (bar_end - bar_start) * np.random.uniform(0.5, 0.9)
            current_time_in_bar = bar_start + time_into_bar
            
            # Get data available up to this point in the bar
            current_orderflow = orderflow_df[(orderflow_df.index >= bar_start) & (orderflow_df.index <= current_time_in_bar)]
            current_book = book_df[(book_df.index >= bar_start) & (book_df.index <= current_time_in_bar)]

            if current_orderflow.empty or current_book.empty:
                continue

            # Simulate current bar data
            kline_bar = kline_df.iloc[i]
            
            # Ensure there is data to select the current price
            if kline_df.index[kline_df.index < current_time_in_bar].empty:
                continue
            current_price = kline_df['close'].loc[kline_df.index < current_time_in_bar].iloc[-1]
            
            current_bar_data = {
                'open': kline_bar['open'],
                'high': max(kline_bar['open'], current_price),
                'low': min(kline_bar['open'], current_price),
                'current': current_price,
                'time_remaining_ratio': 1 - (time_into_bar.total_seconds() / (bar_end - bar_start).total_seconds())
            }
            
            # Prepare features and target
            features = self.microstructure_bridge._prepare_xgb_features(current_bar_data, current_orderflow, current_book)
            target = kline_bar['close']
            
            X_train_list.append(features)
            y_train_list.append(target)

        if not X_train_list:
            print("  ✗ Failed to generate any training samples.")
            self.logger.warning("No training samples generated for XGBoost model.")
            return
        ...
        # Fill any potential NaNs in features
        X_train = X_train.fillna(0)

        print(f"  Generated {len(X_train)} training samples.")
        self.logger.info(f"Training XGBoost with {len(X_train)} samples.")
        
        self.microstructure_bridge.train_xgboost_model(X_train, y_train)
        
        print("  ✓ XGBoost model training complete.")
        self.logger.info("XGBoost model training successful.")

    def save_models(self):
        """Save all trained models to disk for persistence"""
        model_dir = "models"
        os.makedirs(model_dir, exist_ok=True)
        self.logger.info(f"Saving models to directory: {model_dir}")

        # Save XGBoost model
        if self.microstructure_bridge.xgb_model:
            with open(os.path.join(model_dir, "xgb_microstructure_model.pkl"), 'wb') as f:
                pickle.dump(self.microstructure_bridge.xgb_model, f)
            self.logger.info("Saved XGBoost model.")

        # Save CNN model and scaler
        if self.cnn_predictor.is_trained:
            torch.save(self.cnn_predictor.model.state_dict(), os.path.join(model_dir, "cnn_market_model.pth"))
            with open(os.path.join(model_dir, "cnn_scaler.pkl"), 'wb') as f:
                pickle.dump(self.cnn_predictor.scaler, f)
            self.logger.info("Saved CNN model and scaler.")
    ...
    def load_models(self):
        ...
        if not os.path.exists(model_dir):
            print("  ✗ Models directory not found. Cannot load models.")
            self.logger.warning("Models directory not found.")
            return

        print("\n[LOADING PRE-EXISTING MODELS]")
        self.logger.info(f"Loading models from directory: {model_dir}")

        # Load XGBoost model
        xgb_path = os.path.join(model_dir, "xgb_microstructure_model.pkl")
        if os.path.exists(xgb_path):
            with open(xgb_path, 'rb') as f:
                self.microstructure_bridge.xgb_model = pickle.load(f)
            self.logger.info("Loaded XGBoost model.")
            print("  ✓ XGBoost model loaded.")
        else:
            self.logger.warning("XGBoost model file not found.")
            print("  ✗ XGBoost model not found.")

        # Load CNN model and scaler
        cnn_path = os.path.join(model_dir, "cnn_market_model.pth")
        scaler_path = os.path.join(model_dir, "cnn_scaler.pkl")
        if os.path.exists(cnn_path) and os.path.exists(scaler_path):
            # Initialize model architecture before loading state_dict
            self.cnn_predictor.model = MarketPredictionCNN().to(self.cnn_predictor.device)
            self.cnn_predictor.model.load_state_dict(torch.load(cnn_path))
            self.cnn_predictor.model.eval() # Set to evaluation mode
            
            with open(scaler_path, 'rb') as f:
                self.cnn_predictor.scaler = pickle.load(f)
            
            self.cnn_predictor.is_trained = True
            self.logger.info("Loaded CNN model and scaler.")
            print("  ✓ CNN model and scaler loaded.")
        else:
            self.logger.warning("CNN model or scaler file not found.")
            print("  ✗ CNN model or scaler not found.")


if __name__ == '__main__':
    # Check for command-line argument to load models
    load_models_flag = '--load-models' in sys.argv
    
    # Initialize the system
    analysis_system = IntegratedMathematicalAnalysisSystem(load_existing_models=load_models_flag)
